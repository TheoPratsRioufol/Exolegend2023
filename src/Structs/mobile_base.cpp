...
bool go_to(Position cons, Position pos, Gladiator *gladiator)
{
    ...
    if (d > erreurPos)
    {
        ...
        if (abs(reductionAngle(rho - pos.a)) > erreurPos_angle) {
            double consw = kw * reductionAngle(rho - pos.a);
            consvl = -gladiator->robot->getRobotRadius() * consw;
            consvr =  gladiator->robot->getRobotRadius() * consw;
        }
        else {
            int sgn = cos(reductionAngle(rho - pos.a)) >= 0 ? 1 : -1;
            double consv = kv * d * sgn;      // intégrer le signe de d négatif
            consvl = consv;
            consvr = consv;
        }
